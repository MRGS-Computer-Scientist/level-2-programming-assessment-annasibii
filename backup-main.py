...
def change_color():
    ...
    if selected_var.get() == 1:
        ...
        health_radio.config(bg=radio_button_bg_color, fg=radio_button_fg_color)
    elif selected_var.get() == 2:
        health_radio.config(bg="gray", fg="black")
        productivity_radio.config(bg=radio_button_bg_color,
...
aboutus_frame = Frame(window, width=1280, height=700, background='#2C2A64')
...
context_label = Label(
    aboutus_frame,
    text=
    "My Goalie is designed to help assist young people on their journey towards success. It enables young people to goes through a question quiz which leads them to select from recommended personalised goals which they can work and focus on.By visually selecting a goal helps to deepen the understanding of what the user’s goal is and how to achieve it",
    font=("Verdana", 15),
    fg="white",
    bg="#2C2A64")
#Positioning of this text
context_label.place(relx=0.8, rely=0.4, anchor=CENTER)
